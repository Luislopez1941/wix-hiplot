...
const EditorContainers = ({ typeName, dataEditContainer, indexEditSection, selectedTypeSection }: any) => {
  ...
  }, [selectedTypeFontWeight, selectedColor, fontSize, textAlign, content, typeName]);
    ...
    const data = containers?.map((x: any, index: number) => {
      if (index === dataEditContainer?.index) {
        return {
          ...
          imagen: {
            ...x.imagen,
            textAlign: alignment.replace("text_align-", "")
          }
        };
      }
      return x;
  ...
  const handleTitleContainerChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const data = containers?.map((x: any, i: number) => {
      if (i === dataEditContainer?.index) {
        return { ...x, titulo: e.target.value };
      }
      return x;
    });
